...
test.beforeEach(async ({page}) => {
  ...
  await details.fillInputDetails('first','last');
});

test('All fields payment master card', async ({ page }) => {
  await agilewayPayment.checkUseMastercard();
  await agilewayPayment.fillPaymentDetails('1234123412341234','06','2026');
  
  await agilewayPayment.verifyConfirmationHeaderAppears();
});

test('All fields payment visa', async ({ page }) => {
  await agilewayPayment.checkUseVisa();
  await agilewayPayment.fillPaymentDetails('1234123412341234','06','2026');
