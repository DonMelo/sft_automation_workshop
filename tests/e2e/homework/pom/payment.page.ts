...
export class PaymentPage {
    ...
    async validateFlightName(name: string) {
        expect((await this.nameConfirmation.textContent())).toContain(name);
    }
    async validateCardName(name: string) {
        expect((await this.nameField.inputValue())).toContain(name);
    }
    async validateFare(returnType: string, from: string, to: string) {
        expect((await this.fareInfoConfirmation.textContent())).toContain(returnType + " " + from + " to " + to);
    }
}