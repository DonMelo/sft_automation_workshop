...
import { users, messages, tripType, cities, flightDates, headers, passenger } from '../data/testData';
...
test.describe('Passenger Details Page', () => {
  test('User can fill in passenger details', async ({ page }) => {

    await pageManager.passengerDetailsPage.fillAndSubmitPassengerDetails(passenger.firstName,passenger.lastName);
    await pageManager.basePage.waitForPageHeading(headers.paymentPage);
  });

  test('User fills first name but leaves last name blank', async ({ page }) => {
    
    await pageManager.passengerDetailsPage.fillAndSubmitPassengerDetails(passenger.firstName,passenger.lastNameBlank);
    await expect(pageManager.basePage.errorMessage).toHaveText(messages.lastNameRequired);
  });
  ...
  test('User fills last name but leaves first name blank', async ({ page }) => {
    ...
    await pageManager.passengerDetailsPage.fillAndSubmitPassengerDetails(passenger.firstNameBlank,passenger.lastName);
