...
test("User can see fare and fill payment details", async () => {
  ...
  await pageManager.paymentPage.waitForPageHeading(headers.confirmation);
...
test("User cannot submit with empty card details", async () => {
  await pageManager.paymentPage.selectCardType("visa");
  await pageManager.paymentPage.submitPayment();
  await expect(pageManager.paymentPage.errorAlert).toBeVisible();
});

// Should display an error when card type is not selected
test("User cannot submit withouth seletcing card type", async () => {
  await pageManager.paymentPage.fillCardNumber(creditCard.number);
  await pageManager.paymentPage.submitPayment();
  await expect(pageManager.paymentPage.errorAlert).toBeVisible();
});
